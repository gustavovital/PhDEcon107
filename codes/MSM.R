...
for (sentiment_var in sentiment_vars) {
  for (lag in lags) {
    ...
    lm_model <- lm(formula_model, data = data2, na.action = na.exclude)
    ...
    ms_model_p0 <- tryCatch({
      ...
      message(paste("⚠️ Error p=0 in:", model_name))
      return(NULL)
    })
    ...
    ms_model_p1 <- tryCatch({
      ...
      message(paste("⚠️ Error p=1 in:", model_name))
      return(NULL)
    })
