...
sentiment_vars <- names(data)[grepl("^norm_", names(data))]

# Remover icc se por acaso estivesse (não está, mas segurança)
sentiment_vars <- setdiff(sentiment_vars, "icc")

print(sentiment_vars)
...
results_granger <- lapply(sentiment_vars, function(var) {
